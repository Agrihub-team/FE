...
// ==========================================
// 1. SCHEMAS (ZOD) - THAY THẾ CHO IF/ELSE VALIDATE
// ==========================================
...
const requestOtpSchema = z.object({
  resetEmail: z.string().min(1, "Vui lòng nhập Email").email("Email không hợp lệ"),
});
...
export const Login = () => {
  ...
  const [savedResetEmail, setSavedResetEmail] = useState(""); // Lưu tạm email để gửi reset

  // ==========================================
  // 2. KẾT NỐI REACT HOOK FORM
  // ==========================================
  // Form Đăng Nhập
  const { 
    register: regLogin, 
  ...
  const { 
    ...
    reset: resetReqForm
  } = useForm<RequestOtpInputs>({ resolver: zodResolver(requestOtpSchema), mode: "onChange" });
  ...
  } = useForm<ResetPasswordInputs>({ resolver: zodResolver(resetPasswordSchema), mode: "onChange" });
  ...
  const onRequestOtp = async (data: RequestOtpInputs) => {
    try {
      await authService.sendOtp({ email: data.resetEmail, type: 'forgot' });
      setSavedResetEmail(data.resetEmail); // Lưu lại để bước sau xài
      setIsOtpSent(true); 
      toast.success(`Mã xác nhận đã gửi đến ${data.resetEmail}`);
    } catch (error: any) {
      toast.error(error.message || "Lỗi gửi mã xác nhận!");
    }
  };
      ...
      <main className="flex-grow bg-white py-10 px-4 flex flex-col items-center">
        <div className="w-full max-w-[500px] mb-10">
          ...
          <div className="flex justify-center border-b border-gray-200 mb-8">
            ...
            <Link to="/register" className="w-1/2 text-center py-3 text-gray-500 hover:text-[#047857] text-[15px] uppercase transition-colors">
              ĐĂNG KÝ
            </Link>
          </div>

          <h1 className="text-2xl text-center font-normal mb-6 uppercase">ĐĂNG NHẬP</h1>
          ...
          <form onSubmit={handleLoginSubmit(onLogin)} className="space-y-4">
            <div>
              <input
                ...
                placeholder="Email"
                {...regLogin('email')}
                className={`w-full border px-4 py-2.5 rounded text-[14px] outline-none transition-colors 
                  ${loginErrs.email ? 'border-red-500 focus:border-red-500' : 'border-gray-300 focus:border-[#047857]'}`}
              />
              {loginErrs.email && <p className="text-red-500 text-xs mt-1">{loginErrs.email.message}</p>}
            </div>
            ...
            <div className="relative">
              <input
                ...
                {...regLogin('password')}
                className={`w-full border px-4 py-2.5 rounded text-[14px] outline-none transition-colors 
                  ${loginErrs.password ? 'border-red-500 focus:border-red-500' : 'border-gray-300 focus:border-[#047857]'}`}
              />
              <button
                type="button"
              ...
              {loginErrs.password && <p className="text-red-500 text-xs mt-1">{loginErrs.password.message}</p>}
            </div>
          ...
          <div className={`transition-all duration-300 ease-in-out overflow-hidden ${showForgot ? "max-h-[500px] opacity-100 mt-4" : "max-h-0 opacity-0"}`}>
            
            {!isOtpSent ? (
              /* FORM YÊU CẦU OTP */
              <form onSubmit={handleReqSubmit(onRequestOtp)} className="space-y-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                <p className="text-sm text-gray-600 text-center mb-2">Nhập email để nhận mã xác nhận</p>
                <div>
                  <input
                    type="email"
                    placeholder="Email của bạn"
                    {...regReq('resetEmail')}
                    className={`w-full border px-4 py-2.5 rounded text-[14px] outline-none transition-colors bg-white
                      ${reqErrs.resetEmail ? 'border-red-500' : 'border-gray-300 focus:border-[#047857]'}`}
                  />
                  {reqErrs.resetEmail && <p className="text-red-500 text-xs mt-1">{reqErrs.resetEmail.message}</p>}
                </div>
                <button 
                  type="submit"
              ...
              <form onSubmit={handleResetSubmit(onResetPassword)} className="space-y-4 bg-emerald-50 p-4 rounded-lg border border-emerald-200">
                <p className="text-sm text-[#047857] text-center mb-2">Mã đã được gửi đến: <b>{savedResetEmail}</b></p>
                ...
                <div>
                  <input
                    ...
                    maxLength={6}
                    {...regReset('otp')}
                    className={`w-full border px-4 py-2.5 rounded text-[14px] outline-none transition-colors bg-white tracking-widest text-center font-bold
                      ${resetErrs.otp ? 'border-red-500' : 'border-gray-300 focus:border-[#047857]'}`}
                  />
                  {resetErrs.otp && <p className="text-red-500 text-xs mt-1 text-center">{resetErrs.otp.message}</p>}
                </div>
                ...
                <div>
                  <input
                    ...
                    className={`w-full border px-4 py-2.5 rounded text-[14px] outline-none transition-colors bg-white
                      ${resetErrs.newPassword ? 'border-red-500' : 'border-gray-300 focus:border-[#047857]'}`}
                  />
                  {resetErrs.newPassword && <p className="text-red-500 text-xs mt-1">{resetErrs.newPassword.message}</p>}
                </div>
                ...
                >
                  {isResetLoading && <Loader2 className="animate-spin" size={18} />}
