...
export const ProductDetail = () => {
  ...
  const p25 = parseFloat(product.price_bag_25kg) || 0;
  const p50 = parseFloat(product.price_bag_50kg) || 0;
  const pk = parseFloat(product.price_kg) || 0;
  ...
  const handleAddToCart = () => {
    ...
    // Đã cập nhật thuộc tính itemVoucher vào payload của sản phẩm
    addItem({
      ...
      qKg: qtyKg, 
      pKg: pk,
      itemVoucher: isEligible ? selectedVoucher : null // <-- Đẩy trực tiếp voucher vào item
    });
            ...
            <div className="md:w-[58%] w-full">
              ...
              <div className="flex flex-col lg:flex-row gap-6 mb-8 items-stretch">
                <div className="lg:w-1/2 flex flex-col bg-slate-50 border border-gray-300 rounded-xl p-5 shadow-sm">
                   ...
                   <div className="space-y-3 flex-1 flex flex-col justify-center">
                     {[
                       { label: 'Bao 25kg', price: p25, qty: qty25, set: setQty25 },
                       { label: 'Bao 50kg', price: p50, qty: qty50, set: setQty50 },
                       { label: 'Ký lẻ nông sản', price: pk, qty: qtyKg, set: setQtyKg }
                     ].map((item, idx) => (
                ...
                <div className="lg:w-1/2 flex flex-col bg-slate-50 border border-gray-300 rounded-xl p-5 shadow-sm">
                   <p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-5 border-l-4 border-[#047857] pl-3">2. VOUCHER KHUYẾN MÃI</p>
              ...
              <div className="bg-white border border-gray-300 rounded-xl p-8 shadow-sm">
                ...
                <div className="space-y-4 mb-8 border-b border-gray-100 pb-6 font-medium text-[14px]">
                  {qty25 > 0 && <div className="flex justify-between text-gray-600 italic"><span>1 bao 25kg x {p25.toLocaleString()}đ:</span><span>{(qty25*p25).toLocaleString()}đ</span></div>}
                  {qty50 > 0 && <div className="flex justify-between text-gray-600 italic"><span>1 bao 50kg x {p50.toLocaleString()}đ:</span><span>{(qty50*p50).toLocaleString()}đ</span></div>}
                  {qtyKg > 0 && <div className="flex justify-between text-gray-600 italic"><span>1 ký lẻ x {pk.toLocaleString()}đ:</span><span>{(qtyKg*pk).toLocaleString()}đ</span></div>}
                  ...
                  {isEligible && (
                    <div className="flex justify-between items-center text-[14px] font-bold text-[#047857] bg-green-50 p-4 rounded-xl border border-green-100">
                      <span>Voucher giảm giá: <b className="uppercase underline decoration-dotted ml-1">{selectedVoucher.code}</b> <span className="font-normal text-[11px] ml-1 opacity-70">(Đơn từ {Number(selectedVoucher.minAmount).toLocaleString()}đ)</span></span>
              ...
              {relatedProducts.map((p) => (
                <div key={p._id} className="bg-white border border-gray-200 rounded-xl p-6 hover:border-[#047857] hover:shadow-xl transition-all duration-300 flex flex-col h-full overflow-hidden shadow-sm text-center">
                  <Link to={`/products/${p._id}`} className="flex-1">
                    ...
                    <p className="text-[#047857] font-bold text-xl italic mb-6">{(parseFloat(p.price_bag_25kg) || 0).toLocaleString()}đ</p>
                  </Link>
                  <button 
                    onClick={() => {
